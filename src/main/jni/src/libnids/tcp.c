...
#include "../../include/libnids/hash.h"
#include "../../include/utils.h"
#include "../../include/libnids/scan.h"
#include "../../include/tcp.h"
#include "../../include/libnids/checksum.h"
...
static void add_tcp_closing_timeout(struct tcp_stream * a_tcp) {
	...
	newto->prev = 0;
	for (newto->next = to = nids_tcp_timeouts; to; newto->next = to =
			to->next) {
		if (to->a_tcp == a_tcp) {
			free(newto);
			return;
...
static void add_new_tcp(struct tcphdr * this_tcphdr, struct ip * this_iphdr) {
	...
	if (tcp_num > max_stream) {
		...
		if (orig_client_state != TCP_SYN_SENT)
			nids_params.syslog(NIDS_WARN_TCP, NIDS_WARN_TCP_TOOMUCH, ugly_iphdr,
					this_tcphdr);
	}
	a_tcp = free_streams;
	if (!a_tcp) {
...
// we know that if one_loop_less!=0, we have only one callback to notify
	...
	prune_listeners: prev_addr = &a_tcp->listeners;
	i = a_tcp->listeners;
	while (i)
		if (!i->whatto) {
			*prev_addr = i->next;
...
static void add_from_skb(struct tcp_stream * a_tcp, struct half_stream * rcv,
		struct half_stream * snd, u_char *data, int datalen, u_int this_seq,
		char fin, char urg, u_int urg_ptr) {
...
static void tcp_queue(struct tcp_stream * a_tcp, struct tcphdr * this_tcphdr,
		struct half_stream * snd, struct half_stream * rcv, char *data,
		int datalen, int skblen) {
	...
	if (!after(this_seq, EXP_SEQ)) {
		if (after(this_seq + datalen + (this_tcphdr->fin), EXP_SEQ)) { //TODO this_tcphdr->th_flags & TH_FIN
			/* the packet straddles our window end */
			get_ts(this_tcphdr, &snd->curr_ts);
			add_from_skb(a_tcp, rcv, snd, (u_char *) data, datalen, this_seq,
					this_tcphdr->fin, //TODO (this_tcphdr->th_flags & TH_FIN),
					this_tcphdr->urg, //TODO(this_tcphdr->th_flags & TH_URG),
					ntohs(this_tcphdr->urg_ptr) + this_seq - 1); //TODO ntohs(this_tcphdr->th_urp) + this_seq - 1);
			/*
			...
			while (pakiet) {
				...
				if (after(pakiet->seq + pakiet->len + pakiet->fin, EXP_SEQ)) {
					add_from_skb(a_tcp, rcv, snd, pakiet->data, pakiet->len,
							pakiet->seq, pakiet->fin, pakiet->urg,
		...
		pakiet->fin = this_tcphdr->fin; //TODO (this_tcphdr->th_flags & TH_FIN);
		...
		pakiet->seq = this_seq;
		pakiet->urg = this_tcphdr->urg; //TODO (this_tcphdr->th_flags & TH_URG);
		pakiet->urg_ptr = ntohs(
				this_tcphdr->urg_ptr/**TODO this_tcphdr->th_urp*/);
		for (;;) {
...
static void prune_queue(struct half_stream * rcv, struct tcphdr * this_tcphdr) {
	...
	nids_params.syslog(NIDS_WARN_TCP, NIDS_WARN_TCP_BIGQUEUE, ugly_iphdr,
			this_tcphdr);
	while (p) {
		free(p->data);
		tmp = p->next;
...
find_stream(struct tcphdr * this_tcphdr, struct ip * this_iphdr,
	...
	this_addr.source = ntohs(
			this_tcphdr->source /**TODO this_tcphdr->th_sport*/);
	this_addr.dest = ntohs(this_tcphdr->dest /**TODO this_tcphdr->th_dport*/);
	...
	reversed.source = ntohs(this_tcphdr->dest /**TODO this_tcphdr->th_dport*/);
	reversed.dest = ntohs(this_tcphdr->source /**TODO this_tcphdr->th_sport*/);
...
nids_find_tcp_stream(struct tuple4 *addr) {
	...
	for (a_tcp = tcp_stream_table[hash_index];
			a_tcp && memcmp(&a_tcp->addr, addr, sizeof(struct tuple4)); a_tcp =
					a_tcp->next_node)
		;
...
void process_tcp(u_char * data, int skblen) {
	...
	struct tcphdr *this_tcphdr =
			(struct tcphdr *) (data + 4 * this_iphdr->ip_hl);
	int datalen, iplen;
	int from_client = 1;
	unsigned int tmp_ts;
	struct tcp_stream *a_tcp;
	...
	if ((unsigned) iplen < 4 * this_iphdr->ip_hl + sizeof(struct tcphdr)) {
		LOGE("tcp header error 1");
		nids_params.syslog(NIDS_WARN_TCP, NIDS_WARN_TCP_HDR, this_iphdr,
				this_tcphdr);
		return;
	} // ktos sie bawi

	datalen = iplen - 4 * this_iphdr->ip_hl - 4 * this_tcphdr->doff; //TODO this_tcphdr->th_off;

	if (datalen < 0) {
		LOGE("tcp header error 2");
		nids_params.syslog(NIDS_WARN_TCP, NIDS_WARN_TCP_HDR, this_iphdr,
				this_tcphdr);
		return;
	} // ktos sie bawi
	...
	if ((this_iphdr->ip_src.s_addr | this_iphdr->ip_dst.s_addr) == 0) {
		LOGE("tcp header error 3");
		nids_params.syslog(NIDS_WARN_TCP, NIDS_WARN_TCP_HDR, this_iphdr,
				this_tcphdr);
		return;
	}
	if (!(this_tcphdr->ack /**TODO this_tcphdr->th_flags & TH_ACK*/))
		detect_scan(this_iphdr);
	if (!nids_params.n_tcp_streams)
		return;
	if (my_tcp_check(this_tcphdr, iplen - 4 * this_iphdr->ip_hl,
			this_iphdr->ip_src.s_addr, this_iphdr->ip_dst.s_addr)) {
		LOGE("tcp header error 5");
		nids_params.syslog(NIDS_WARN_TCP, NIDS_WARN_TCP_HDR, this_iphdr,
				this_tcphdr);
		return;
	}
...
//ECN
	...
	if (!(a_tcp = find_stream(this_tcphdr, this_iphdr, &from_client))) {
		if ((this_tcphdr->syn /**TODO this_tcphdr->th_flags & TH_SYN*/)
				&& !(this_tcphdr->ack /**TODO this_tcphdr->th_flags & TH_ACK*/)
				&& !(this_tcphdr->rst /**TODO this_tcphdr->th_flags & TH_RST*/)) {
	...
	if ((this_tcphdr->syn /**this_tcphdr->th_flags & TH_SYN*/)) {
		...
		if (a_tcp->client.seq
				!= ntohl(this_tcphdr->ack_seq/**TODOthis_tcphdr->th_ack*/)) {
			return;
		}
		a_tcp->server.state = TCP_SYN_RECV;
		a_tcp->server.seq = ntohl(this_tcphdr->seq/**TODOthis_tcphdr->th_seq*/)
				+ 1;
		a_tcp->server.first_data_seq = a_tcp->server.seq;
		a_tcp->server.ack_seq = ntohl(
				this_tcphdr->ack_seq /**TODOthis_tcphdr->th_ack*/);
		a_tcp->server.window = ntohs(
				this_tcphdr->window /**TODOthis_tcphdr->th_win*/);
		...
		if (a_tcp->client.wscale_on) {
			a_tcp->server.wscale_on = get_wscale(this_tcphdr,
					&a_tcp->server.wscale);
	...
	if (!(!datalen
			&& ntohl(this_tcphdr->seq /**TODO this_tcphdr->th_seq*/)
					== rcv->ack_seq)
			&& (!before(ntohl(this_tcphdr->seq /**TODO this_tcphdr->th_seq*/),
					rcv->ack_seq + rcv->window * rcv->wscale)
					|| before(
							ntohl(
									this_tcphdr->seq /**TODO this_tcphdr->th_seq*/)
									+ datalen, rcv->ack_seq))) {
		return;
	}

	if ((this_tcphdr->rst /**TODO this_tcphdr->th_flags & TH_RST*/)) {
	...
	if ((this_tcphdr->ack/**TODO this_tcphdr->th_flags & TH_ACK*/)) {
		if (from_client && a_tcp->client.state == TCP_SYN_SENT
				&& a_tcp->server.state == TCP_SYN_RECV) {
			if (ntohl(this_tcphdr->ack_seq/**TODO this_tcphdr->th_ack*/)
					== a_tcp->server.seq) {
				a_tcp->client.state = TCP_ESTABLISHED;
				a_tcp->client.ack_seq = ntohl(
						this_tcphdr->ack_seq/**TODO this_tcphdr->th_ack*/);
				{
					struct proc_node *i;
					struct lurker_node *j;
					void *data;
	...
	if ((this_tcphdr->ack/**TODO this_tcphdr->th_flags & TH_ACK*/)) {
		handle_ack(snd,
				ntohl(this_tcphdr->ack_seq/**TODO this_tcphdr->th_ack*/));
	...
	if (datalen + (this_tcphdr->fin /**TODO this_tcphdr->th_flags & TH_FIN*/)
			> 0)
		tcp_queue(a_tcp, this_tcphdr, snd, rcv,
				(char *) (this_tcphdr) + 4 * this_tcphdr->doff, //TODO this_tcphdr->th_off,
				datalen, skblen);
	snd->window = ntohs(this_tcphdr->window/**TODO this_tcphdr->th_win*/);
...
int tcp_init(int size) {
	...
	streams_pool = (struct tcp_stream *) malloc(
			(max_stream + 1) * sizeof(struct tcp_stream));
	if (!streams_pool) {
		nids_params.no_mem("tcp_init");
		return -1;
	}
