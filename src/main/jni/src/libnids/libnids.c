...
#endif
#include <stdlib.h>
#include "../../include/libnids/tcp.h"
#include "../../pcap-bpf.h"
#include "../../include/libnids/ip_fragment.h"
...
static void nids_no_mem(char *func);
...
static void nids_syslog(int type, int errnum, struct ip *iph, void *data) {
	...
	if (errnum != NIDS_WARN_IP_HDR) {
	    ...
	    LOGE("%s, packet (apparently) from %s to %s\n",
    ...
    default:
    	LOGE("Unknown warning number ?\n");
    }
}

static void nids_no_mem(char *func) {
	LOGE("Out of memory in %s", func);
}
...
{
	LOGE("nids_pcap_handler, catch a packet");
    u_char *data_aligned;
#ifdef HAVE_LIBGTHREAD_2_0
...
static void process_udp(char *data)
{
    ...
    if (udph->check /**TODO udph->uh_sum*/ && my_udp_check
	((void *) udph, ulen, iph->ip_src.s_addr,
	 iph->ip_dst.s_addr)) return;
    addr.source = ntohs(udph->source/**TODO udph->UH_SPORT*/);
    addr.dest = ntohs(udph->dest/**TODO udph->UH_DPORT*/);
...
static int open_live()
{
	...
	return 0;
 #if 0 //TODO #ifdef __linux__
    if (!strcmp(device, "any") && nids_params.promisc
	&&  TODO !set_all_promisc()) {
	nids_errbuf[0] = 0;
...
int nids_init()
{
	...
	if (pcap_compile(desc, &fcode, nids_params.pcap_filter, 1, mask) < 0) {
		//if filter is error,notify app and start pcap with no filter
		filter_error = 1;
		nids_params.pcap_filter = "";
		pcap_compile(desc, &fcode, nids_params.pcap_filter, 1, mask);
	}
